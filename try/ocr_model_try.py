...
def retrain():
    ...
    ds_val = tf.contrib.data.TFRecordDataset('../dataset/tfdata/test.tfrecord', 'GZIP')
    ds_val = ds_val.map(parse_tfrecord_function_with_raw)
    ds_val = ds_val.batch(1)
    iterator_val = ds_val.make_one_shot_iterator()
    inputs_val = iterator_val.get_next()

    model_ocr = OCRModel(6516, 120)

    vocab = {}
    loadVocab('../dataset/unicode_chars.txt', vocab)
    with tf.Session() as sess:
        saver = tf.train.import_meta_graph(os.path.join(models_dir, 'ocr-2.meta'))
        ...
        op_onlinereference = graph.get_tensor_by_name('onlineInferenceModel:0')
        inp_x = graph.get_tensor_by_name('inp_x:0')

        X = inp_x
        X = 255 - X
        xL = model_ocr.length(X)
        op_xw_plus_b = graph.get_tensor_by_name('xw_plus_b:0')
        op_reshape = tf.reshape(op_xw_plus_b, [-1, TIME_STEPS, 6517])
        decoded, _ = tf.nn.ctc_beam_search_decoder(
            tf.transpose(op_reshape, [1, 0, 2]), xL, beam_width=5, top_paths=1, merge_repeated=True
        )
        op_another = tf.sparse_tensor_to_dense(decoded[0], name='onlineInferenceModel', default_value=REAL_LABEL_NUM)

        fw_kernel = tf.graph.get_tensor_by_name('bilstm/bidirectional_rnn/fw/lstm_cell/kernel:0')
        ...
        for i in range(1000):
            ...
            o_xL = sess.run(xL, feed_dict={inp_x:inputs[0]})
            o_another = sess.run(op_xw_plus_b, feed_dict={inp_x:inputs[0]})
...
def test_train():
    ...
    train_by_epochs('../dataset/tfdata-0-10/test.tfrecord',
          '../dataset/tfdata-0-10/test.tfrecord',
          model, 100, 2048, 128, 0.001, 10000
          )

def test_train_noepochs():
    model = OCRModel(numChars=6516, lstmHidden=120)
    train('../dataset/tfdata-0-10/train.tfrecord',
                    '../dataset/tfdata-0-10/val.tfrecord',
                    model, 100, 2048, 384, 0.001, 1000000
                    )
